...
import shui.contracts.choice.RadioGroupable;
import shui.contracts.choice.Toggleable;
import shui.contracts.visual.VisualState;
import shui.delegates.choice.RadioGroupDelegate;
...
/**
 * RadioButton Shui desacoplado de JRadioButton.
 */
public class ShRadioButton extends BaseContainer
        implements Toggleable, RadioGroupable, RadioGroupDelegate.RadioMember {
    ...
    private final RadioGroupDelegate<ShRadioButton> groupDelegate = new RadioGroupDelegate<>(this);

    private String groupName = "default";

    public ShRadioButton() {
        super(4, EMPTY_BG);
        initUI();
    }
    ...
    private void initUI() {
        ...
        setText("RadioButton");
        groupDelegate.register(groupName);
        installHandlers();
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (toggleDelegate.isSelected()) {
            ...
            int dx = x + (size - dot) / 2;
            int dy = y + (size - dot) / 2;
            g2.setColor(toggleDelegate.getIndicatorColor());
    ...
    public void setSelected(boolean selected) {
        boolean changed = toggleDelegate.setSelected(selected);
        if (selected) {
            groupDelegate.selectOnlyCurrent();
        }
        if (changed) {
            repaint();
        }
    ...
    public void setGroupName(String groupName) {
        ...
        groupDelegate.register(this.groupName);
    }
    ...
    @Override
    public void setSelectedFromGroup(boolean selected) {
